...
**Collision detection.** If a server observes a `/_matrix/key/v2/server` response from a remote server where a Key ID that was previously associated with public key `A` is now associated with a different public key `B`, the receiving server MUST:
