...
- **Escape Hatch Conditions (MUST):** To invoke this escape hatch, the modification MUST meet specific incompatibility conditions (e.g., containing structural JSON alterations that would otherwise cause a standard mainnet parser to crash or throw signature validation errors). Ad-hoc suffixes that do not append to an owned rDNS or MSC base namespace are not permitted.
