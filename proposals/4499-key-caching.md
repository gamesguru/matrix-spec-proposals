...
<!-- synapse-derived: complement coverage directly exercises this request-auth
scope in TestFederationRequestAuthenticationKeyScope -->
...
<!-- synapse-derived: complement coverage keeps the outer X-Matrix request on a
currently valid key while separately exercising historical PDU verification on
old/expired keys in testMSC4499KeyHistoricalEventVerification and
testMSC4499KeyLostKeyPublicationHistoricalVerification -->
...
bytes, a 32-byte `SHA-256` digest of the key body, and a `first_seen` timestamp
recording when the receiver itself established the binding — kept for operator
forensics and for a future proposal to build eviction or corroboration policy on
top of without a schema change; this MSC's own rules do not read it — on the
order of 315 bytes per record, far smaller than a retained verification entry.
It exists to survive eviction of retired-key verification material so a future
body reusing an evicted key ID is still checked against what was first seen,
closing the collision-blind window that motivates permanent retention in the
first place. Because that guarantee depends on the binding never being evicted
for a genuinely-seen key ID, it MUST NOT be pruned the way retired-key
verification material is — evicting a digest binding to make room for a new one
reopens exactly the TOFU window this record exists to close.
