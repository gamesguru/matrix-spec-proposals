...
- `redactable_content_hash`: canonical hash of the event body fields that are
  stripped during redaction;
- `content_hash`: the inner Merkle hash combining `redacted_content_hash` and
  `redactable_content_hash`, computed with the `inner_hash` construction below.
...
Reference implementations of the causal trie, its inclusion/non-inclusion proof
generation, and verification ship in `gomatrixcrypto` (`merkle.CausalSet`,
`InclusionProof`/`NonInclusionProof`,
`VerifyCausalInclusion`/`VerifyCausalNonInclusion`) and `rezzy`
(`merkle::causal::CausalSet`). The header tree's own `leaf_paths` sibling
extraction and verification (used below) ship as `gomatrixcrypto`'s
`merkle.LeafPath`/`VerifyLeafPath` and `rezzy`'s
`merkle::leaf_path`/`merkle::verify_leaf_path`.

#### End-to-end redacted event vector

The following worked example, generated by `cmd/redacted-event-vectors` in
`gomatrixcrypto`, computes one `m.room.member` event's `event_root` and event ID
both before and after redaction, and confirms they are identical: the server
retains only the 32-byte `redactable_content_hash` after dropping the censored
`third_party_invite.display_name`, and reconstructs the same `content_hash` from
that retained hash plus the still-held `redacted_content`. It also proves the
header's `sender_domain` leaf and one `causal_set` inclusion, tying together
every primitive in this document over a single concrete event:

```text
[msc4511-redacted-event-vector]
event_type = m.room.member
redacted_content = {"membership":"invite","third_party_invite":{"signed":{"mxid":"@bob:example.org","token":"abc"}}}
redactable_content_before_redaction = {"third_party_invite":{"display_name":"Bob"}}
redacted_content_hash_hex = 647157b56dbb7fd191aa2315995dd33cebd5d72d6917a36a3c529ff96126dc7f
redactable_content_hash_hex = d99e3c41773e0314030b5dbadcbe38dba2020b885660d9cb96181d54b4f326e0
content_hash_hex = dd764f7298dde9098ef8296be383c63b2c235544d88ae8cd1d121cf314f6d521
event_root_before_redaction_hex = f2c2f939107810b70d8ab2f8c49e7f3393280986a521df3c6bd30efc778c804d
event_id_before_redaction = $8sL5ORB4ELcNirL4xJ5_M5MoCYalId88a9MO_HeMgE0
event_root_after_redaction_hex = f2c2f939107810b70d8ab2f8c49e7f3393280986a521df3c6bd30efc778c804d
event_id_after_redaction = $8sL5ORB4ELcNirL4xJ5_M5MoCYalId88a9MO_HeMgE0
event_id_unchanged_by_redaction = true
sender_domain_leaf_path_matches_header_root = true
sender_domain_leaf_path_steps = 3
causal_inclusion_proof_ok = true
causal_inclusion_proof_verifies = true
```

`event_id_before_redaction` and `event_id_after_redaction` are identical,
demonstrating the redacted/redactable `content_hash` split's purpose: a verifier
who only ever sees the post-redaction event can still confirm this is the same
event, without ever learning `display_name`.
...
MAY define `redactable_content` as canonical `null`; `redacted_content_hash` and
`redactable_content_hash` are computed the same way regardless of whether either
side is empty.
...
redacted/redactable content partition, signature migration, and State DAG
...
### Redacted/redactable content partition
