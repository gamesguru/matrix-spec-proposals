...
We conclude with a concrete protocol, a resident 17 KB per-room structure that
answers all reconciliations with $\Delta \lesssim 300$ with no disk reads and no
round-trip risk, and a self-stabilization theorem identifying exactly which
...
> **Theorem 1 (Dichotomy).** Let $\phi$ be a digest homomorphism. Exactly one
> of:
>
...
_Proof._ (G) is immediate. For (S): idempotence plus invertibility forces
...
>    extremities, stopped at any point, satisfies this by construction.
...
$A \cup (B\setminus A) = A \cup B$, closed by Proposition 1. (2): a backward
traversal maintains the invariant that every visited node's unvisited ancestors
are frontier nodes, and the frontier is by definition not yet claimed as
recovered. (3): counterexample by taking $R$ to exclude a single minimal element
of $B \setminus A$. $\square$
...
>
> Then from any state, $\Delta \to 0$ with probability 1, in $O(1)$ rounds
> against a fixed peer holding the missing events and $O(\log N)$ rounds against
> the room as a whole.

_Proof sketch._ (H1) removes absorbing holes: every divergent bucket is examined
every round, at $\Theta(b)$ cost independent of depth. (H2) guarantees each
round makes progress or triggers a strictly larger capacity, and by Corollary
3.1 the total cost of the escalation chain is bounded by twice the terminal
capacity. (H3) guarantees the requester eventually samples a peer that holds
$e^\ast$; combined with pull-based epidemic convergence (§11.1) the room-wide
...
- **Resident size:** $256 \times 8 \times 8\,\mathrm{B} = 16\ \mathrm{KB}$, plus
  1 KB of counts. **17 KB per room.**
- **Update cost:** 8 carry-less multiplications and 8 XORs. On any x86-64 with
  `PCLMULQDQ`, on the order of 10 ns — comparable to the hash of the event ID
  itself, and negligible against the database write it accompanies.
- **Coverage:** every reconciliation with $\Delta \lesssim 300$ is answered from
  memory, with **zero disk reads** and no scan of the event store.
...
covers 5,000 events, and fails silently. The structure above is 17 KB per room,
covers the entire frame, is exact, and self-verifies. The exact mechanism is
_cheaper in every dimension that scales_ and more expensive only in a fixed 11
KB.
...
> **Proposition 3.** Given a target $\tau \in \mathbb{F}_2^{\ell}$ and any
> $m > \ell$ candidate events, an adversary finds a subset $S$ with
> $\chi(S) = \tau$ by Gaussian elimination over $\mathbb{F}_2$, in $O(\ell^2 m)$
> time.
...
- **Buckets and sketches:** use $\ell$-bit short identifiers derived with a
  **per-link salt**, $h_K(e)$. BIP-330 uses 32–64 bits with independent per-link
  salts precisely to prevent an adversary from grinding identifiers into
  collisions. Without the salt, short identifiers are grindable offline; with
  it, the adversary must attack each link separately and cannot precompute.
...
8. $\Delta$ need not be guessed. Bucket counts bound it deterministically and
   ...
   (Prop 2, §7.2)
9. The resulting structure is 17 KB resident per room, updated in ~10 ns per
   event, and answers $\Delta \lesssim 300$ from memory with no disk reads. (§9)
