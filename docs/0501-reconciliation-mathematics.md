...
with $$n=2^\ell$$ yields the second inequality. Since $$d$$ is unbounded, no
fixed message size can reconcile every possible difference exactly. A correct
system must adapt by extending its sketch, partitioning the difference,
streaming a rateless representation, or falling back to traversal/backfill.
$$\square$$

## 3. Causal Closure

Let $$\Phi$$ be an antichain of frame anchors. Define the frame generated by
those anchors as
...
For $$R \subseteq B \\ A$$, the recovered set $$A \cup R$$ is closed if and only
if
...
parents in $$A$$, and every event in $$R$$ has its required parents in
$$A \cup R$$. Thus the union is closed. Conversely, if the union is closed,
every parent of each $$e \in R$$ must belong to the union, which is exactly the
...
Let $$D = B \\ A$$ be the missing set and let $$a_e$$ be the number of ancestors
of $$e$$ that also lie in $$D$$. Suppose each missing event is independently
omitted with false-positive probability $$\varepsilon$$.
...
satisfies $$L \leq \Delta$$, with equality exactly when every bucket is
one-sided. Provisioning can therefore use the count residual for the common path
and independently bounded sketches for differing buckets.
...
section, the per-trial rate $$1/b$$ is known and the number of trials $$\Delta$$
is estimated from the observed empty fraction. In Section 11.1, the useful-peer
...
$$X \sim \operatorname{Pois}(\Delta/b)$$. If each bucket has extraction capacity
$$k$$, then
